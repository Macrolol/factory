...
<div class="inline-flex flex-col items-center justify-center px-3 m-1 rounded-md
            only-of-type:rounded-full last-of-type:rounded-r-full transition-all duration-500 first:rounded-l-full
            {current ? 'bg-blue-500 hover:bg-blue-700 text-white -translate-y-1' :
                          'bg-gray-100 hover:bg-gray-200 dark:hover:bg-gray-800'}">
<button on:click={() => {current ? taskStore.unfocusTask() : taskStore.focusTask(task)}}
...
<div role="tooltip"
         class="absolute bottom-full z-10 px-3 text-sm font-medium text-white transition-opacity duration-300
                bg-gray-900 rounded-lg shadow-sm {hover ? 'opacity-100' : 'opacity-0'} dark:bg-gray-700">
        {task.name}
</div>
</div>
